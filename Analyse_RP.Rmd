...
```{r}
medCL <- stan_glm(sum_diff ~ R001 + CL, 
                data = daten2, 
                refresh = 0)
parameters(medCL)
```
```{r}
m1 <- mediate(lm3_1, medCL_1, treat = 'R001', mediator = 'CL', boot = TRUE, sims = 500)
```
```{r}
lm3_1 <- lm(CL ~ R001, 
                data = daten2)
medCL_1 <- lm(sum_diff ~ CL + R001, 
              data = daten2)
```
...
```{r}
mediate_plot(sum_diff ~ CL + R001, data = daten2)
```
## Fazit
